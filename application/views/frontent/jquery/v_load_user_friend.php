...
?>
<?php $check_nd = ''; ?>
<?php if(count($friend_group) != 0){ ?>
<?php foreach ($friend_group as $key => $value_friend): ?>
   ...
   <a href="javascript:void(0)" onclick="chat_friend(<?= $value_friend; ?>)" class="relative flex items-center gap-4 p-2 duration-200 rounded-xl hover:bg-secondery">
      ...
      <div class="flex-1 min-w-0">
         ...
         <?php if(isset($view_group_mess)){ ?>
            <?php $check_nd = $view_group_mess['content_end']; ?>
            <?php if(strlen($view_group_mess['content_end']) > 23){
               $dot = ' ...';
            }else{
               $dot = '';
            } ?>
         <div class="font-medium overflow-hidden text-ellipsis text-sm whitespace-nowrap"><?php if(isset($view_group_mess)){ echo $view_group_mess['content_end'].$dot; }else{ echo 'No messages'; } ?></div>
      <?php } ?>
      </div>
   </a>
<?php endforeach ?>
<?php }else{ ?>
   <h4 style="padding: 10px 20px;">No friend</h4>
<?php } ?>
